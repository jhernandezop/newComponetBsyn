...
class App extends Component {
  ...
  constructor(props) {
    ...
    this.state = {  
       ...
       xmlOverlays:[{"ID_ETAPA_PROCESO":"SCAN_OK", "NOTA":"AQUI HAY 2 FICHAS" },{"ID_ETAPA_PROCESO":"sid-D7F237E8-56D0-4283-A3CE-4F0EFE446138", "NOTA":"AQUI HAY 1 FICHAS" }]
     }
  ...
  interfazExpandida() {
    console.log(this.state.expandida)
    this.setState(state => ({
  ...
  estadoLogin() {
    console.log(this.state.estadoLogin)
    this.setState(state => ({
  ...
  actualizarOverlayXml(overlays) {
    ...
    this.setState({xmlOverlays: [{"ID_ETAPA_PROCESO":"SCAN_OK", "NOTA":overlays }]});
  ...
  render() {
    ...
    const opciones = [{opcion:"a", funcion:"x"},{opcion:"b", funcion:"x"},{opcion:"c", funcion:"x"},{opcion:"d", funcion:"x"}]
      ...
      return (
        <div className="container-fluid h-100">
          <div className="row h-100">
                <div className="col-2" style={{display: (this.state.expandida ? 'block' : 'none')}}>
                   <div className="row h-100">
                      <div id="chart" className="col-12 h-25">

                      </div>
                      <div className="col-12 h-75">
                        <ConstruirFichas fichas={this.state.fichas} grupos={this.state.grupos} actualizarOverlayXml={this.actualizarOverlayXml}/> 
                      </div>
                    </div>
                </div>
                <div className={this.state.expandida ? 'col-10' : 'col-12'}>
                  <div className="row h-100">
                      <div className="col-12 h-25">
                         <div className="row h-100">
                          <div className="col-12 h-25">
                              <OpcioneDeNavegacion  opciones={opciones} interfazExpandida={this.interfazExpandida} estado={this.state.expandida}/>

                          </div>
                          <div className="col-12 h-75">
                            <div className="row">
                             <BpmnViewer xmlProceso={this.state.xmlProceso} xmlOverlays={this.state.xmlOverlays}/>
                            </div>
                          </div>
                        </div>
                      </div>
...
class OpcioneDeNavegacion extends Component {
  ...
  render() {
    ...
    const listItems = opciones.map((number) =>
      <li key={number.opcion}  className="nav-item" >
        <a className="nav-link" >{number.opcion}</a>
      </li>
    );
    ...
    return (
      <nav className="navbar navbar-light bg-light">
        <li className="navbar-brand">
           <a onClick={this.interfazExpandida}>  
                {this.props.estado ? <i className="fas fa-angle-left"></i> : <i className="fas fa-angle-right"></i>}
            </a>
        </li>
